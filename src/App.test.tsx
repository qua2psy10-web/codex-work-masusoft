...
describe('集水マス構造計算UI', () => {
  ...
  it('計算実行後に4項目の支配判定と結果表を表示する', async () => {
    ...
    expect(screen.getByText('部分 83.6%')).toBeInTheDocument()
    expect(screen.getByText('75.24 / 150.00')).not.toHaveClass('cause-cell')
    expect(screen.getByText('240.75 / 150.00')).toHaveClass('cause-cell')
    screen.getAllByText('0.00 kN/m²').forEach((cell) => expect(cell).not.toHaveClass('cause-cell'))
