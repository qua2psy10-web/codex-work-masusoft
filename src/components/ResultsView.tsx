...
function governingStatus(result: CalculationResult, project: ProjectV1, kind: 'uplift' | 'bearing' | 'wall' | 'base') {
  ...
  if (kind === 'bearing') return { caseId, status: item.stability.status }
...
function StabilityTable({ project, result }: { project: ProjectV1; result: CalculationResult }) {
  return <div className="result-table-wrap"><table className="result-table stability-table"><thead><tr><th>ケース</th><th>抵抗重量</th><th>浮力</th><th>安全率 / 必要値</th><th>鉛直合力</th><th>偏心量</th><th>qmin</th><th>qmax / qa</th><th>判定</th></tr></thead><tbody>{caseOrder.map((id) => { const item = result.cases[id]; return <tr key={id}><td>{item.label}</td><td>{format(item.stability.resistingWeight)} kN</td><td>{format(item.stability.upliftForce)} kN</td><td className={item.stability.upliftSafetyFactor < project.basic.upliftSafetyFactor ? 'cause-cell' : ''}>{format(item.stability.upliftSafetyFactor)} / {format(project.basic.upliftSafetyFactor)}</td><td className={item.stability.verticalResultant <= 0 ? 'cause-cell' : ''}>{format(item.stability.verticalResultant)} kN</td><td>{format(item.stability.eccentricity, 3)} m</td><td className={item.stability.bearingMin < 0 ? 'cause-cell' : ''}>{format(item.stability.bearingMin)} kN/m²</td><td className={item.stability.bearingMax > item.stability.allowableBearing ? 'cause-cell' : ''}>{format(item.stability.bearingMax)} / {format(item.stability.allowableBearing)}</td><td><b className={`status-${item.stability.status}`}><StatusIcon status={item.stability.status} />{statusText(item.stability.status)}</b></td></tr> })}</tbody></table></div>
}
